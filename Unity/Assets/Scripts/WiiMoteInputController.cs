...
public class WiiMoteInputController : MonoBehaviour {
...
*/
		...
		if (showDebugGUI) {
			...
			for (int i=0; i<=wiimoteCount-1; i++) {
				...
				/*				    float temp_x = ((ir_x + 1.0f)/ 2.0f) * Screen.width;
				    float temp_y = Screen.height - (((ir_y + 1.0f)/ 2.0f) * Screen.height);*/
				float temp_x = (Screen.width * 0.5f) + ir_x * Screen.width * 0.5f;
				float temp_y = Screen.height - (ir_y * Screen.height);
	...
	// Handle Axis requests
	public float GetAxis(String axis) {
		switch (axis) {
		case "Mouse X":
			// Prevent offcenter analogs from moving camera
			float analogStickX = Mathf.Lerp(-.3f,.3f,(nsx-30)/200);
			return Mathf.Abs(analogStickX) > .05 ? analogStickX : 0;
			break;
		case "Mouse Y":
			return 0;
			break;
		case "Vertical":
			// Take either analog or button input
			if (!wiimote_getButtonUp(0) && !wiimote_getButtonDown(0))
				return Mathf.Lerp(-1f,1f,(nsy-30)/200);
			else
				if (wiimote_getButtonDown(0) && !wiimote_getButtonUp(0))
					return -1;
			...
			break;
		case "Horizontal":
			if (wiimote_getButtonLeft(0) && !wiimote_getButtonRight(0))
				return -1;
			...
			break;
		case "Roll":
			return Mathf.Clamp(wiimote_getRoll(0),-14,14);
			break;
		case "Pitch":
			return Mathf.Clamp(wiimote_getPitch(0),-70, 70);
			break;
		case "Yaw":
			if (wiimote_getIrX(0) == -100)
				return lastYaw;
			lastYaw = Mathf.Clamp(wiimote_getIrX(0),-1,1);
			return lastYaw;
			break;
		default:
				return 0.0f;
